import Section from './Section';
import Heading from './Heading';
import { roadmap } from '../constants';
import { check2, grid, loading1 } from '../assets';
import TagLine from './TagLine';
import { Gradient } from './design/Roadmap';
import Button from './Button';

function AppRoadmap() {
  return (
    <Section className="overflow-hidden flex justify-center" id="roadmap">
      <div className="container md:pb-10">
        <Heading tag="Ready to get started" title="What we’re working on" />

        <div className="relative grid gap-6 md:grid-cols-2 md:gap-4 md:pb-[7rem]">
          {roadmap.map((item) => {
            const status = item.status === 'done' ? 'Done' : 'In progress';

            return (
              <div
                style={
                  item.colorful === 2
                    ? {
                        background:
                          'conic-gradient(from 225deg, #FFC876, #79FFF7, #9F53FF, #FF98E2, #FFC876)',
                      }
                    : { background: '#52525b' }
                }
                className={`md:flex even:md:translate-y-[7rem] p-0.25 rounded-[2.5rem]`}
                key={item.id}
              >
                <div className="relative p-8 bg-neutral-800 rounded-[2.4375rem] overflow-hidden xl:p-15">
                  <div className="absolute top-0 left-0 max-w-full">
                    <img className="w-full" src={grid} width={550} height={550} alt="Grid" />
                  </div>
                  <div className="relative z-1">
                    <div className="flex items-center justify-between max-w-[27rem] mb-8 md:mb-20">
                      <TagLine>{item.date}</TagLine>

                      <div className="flex items-center px-4 py-1 bg-neutral-100 rounded text-neutral-800">
                        <img
                          className="mr-2.5"
                          src={item.status === 'done' ? check2 : loading1}
                          width={16}
                          height={16}
                          alt={status}
                        />
                        <div className="tagline">{status}</div>
                      </div>
                    </div>

                    <div className="mb-10 -my-10 -mx-15">
                      <img
                        className="w-full"
                        src={item.imageUrl}
                        width={628}
                        height={426}
                        alt={item.title}
                      />
                    </div>
                    <h4 className="h4 mb-4">{item.title}</h4>
                    <p className="body-2 text-neutral-400">{item.text}</p>
                  </div>
                </div>
              </div>
            );
          })}

          <Gradient />
        </div>

        <div className="flex justify-center mt-12 md:mt-15 xl:mt-20">
          <Button className={'inline-flex'} href="/roadmap">
            Our roadmap
          </Button>
        </div>
      </div>
    </Section>
  );
}

export default AppRoadmap;
